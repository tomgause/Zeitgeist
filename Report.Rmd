...
```
We observe a $p$-value of about 0.17437, meaning that we have about a 17% chance of observing something more extreme than the test statistic of our sample `actual.test.stat' given that the proportion of students in each class is randomly sampled from the Middlebury population. This is not rare enough to reject the null hypothesis because the $p$-value is larger than $\alpha^{*}$
...
mean(test.stat >= actual.test.stat)
```
Here, we get a $p$-value of about 0.04467 meaning there is about a 4% chance of observing something more rare than our sample's test statistic `actual.test.stat` given that Zeitgeist is randomly sampled from the administration's population data. This is still larger than $\alpha^*$, so we do not have enough evidence to reject the null hypothesis.
