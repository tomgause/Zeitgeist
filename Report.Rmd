...
# test statistic is the sum of ((actual - expected)^2)/expected
actual.test.stat <- sum(((Class.Data.Raw$Zeitgeist - expected.values)^2)/expected.values)
...
for(i in 1:N){
  ...
  test.stat[i] <- sum(((actual.samp.values - expected.values)^2)/expected.values)
}
...
```
We observe a $p$-value of about 0.15317, meaning that we have about a 15% chance of observing something more extreme than the test statistic of our sample `actual.test.stat' given that the proportion of students in each class is randomly sampled from the Middlebury population.
...
samp.size <- sum(Geography.Data.Raw$Zeitgeist)
...
actual.test.stat <- sum(((Geography.Data.Raw$Zeitgeist[1:8] - expected.values)^2)/expected.values)
...
mean(test.stat >= actual.test.stat)
```
Here, we get a $p$-value of about 0.32251 meaning there is about a 32% chance of observing our sample's test statistic `actual.test.stat` given that Zeitgeist is randomly sampled from the administration's population data.
